...
(function(){setTimeout(function(){
  ...
  const fp=Array.from({length:10},(_,i)=>{const r=document.getElementById('g'+i).getBoundingClientRect();return{x:r.left-sr.left,y:r.top-sr.top,w:r.width,h:r.height};});
  ...
  function reset(){ls.forEach(el=>{el.style.opacity='0';el.style.transform='';});ls.forEach((el,i)=>{el.style.left=fp[i].x+'px';});ls[0].style.left=wCx+'px';dot.style.opacity='0';}
  let pi=0,ps=null;
  function frame(ts){
    if(!ps)ps=ts;const el=ts-ps,ph=PHASES[pi],t=Math.min(el/ph.dur,1),et=eO(t),ARC=36;
    switch(ph.name){
      ...
      case 'pause':ls.forEach(el=>{el.style.opacity='0';});dot.style.opacity='0';break;
    }
    if(t>=1){pi=(pi+1)%PHASES.length;ps=ts;if(pi===0)reset();}
  ...
  reset();requestAnimationFrame(frame);
    ...
    with col_r:
        st.markdown(
            "<h2 style='color:#76b900;border-bottom:1px solid rgba(118,185,0,0.2);"
            "padding-bottom:8px;font-family:Inter,sans-serif;letter-spacing:0.06em;"
            "text-transform:uppercase;font-size:14px;margin-bottom:12px'>Sign In</h2>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='color:#6b7280;font-size:13px;margin-bottom:20px'>"
            "Returning user? Sign in with your User ID and email. "
        ...
        with st.form("login_form"):
            ...
            display_name = st.text_input("Display Name",   placeholder="Jane Smith (new accounts only)")
